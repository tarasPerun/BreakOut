 ...
 * The velocity of the ball increases with each broken brick.
 *
 ...
 * The methods .read and .play of Princeton University class StdAudio were used.
 */
public class BreakoutExt extends WindowProgram {
    ...
    // Velocity increase factor, vary from 1.005 to 1.01
    private static final double INCREASE_FACTOR = 1.01;
    ...
    /**
     * Outputs the score label with specified content, color and location.
     *
     * @param color The score label color.
     * @param score The game result, integer value.
     */
    private void putScoreLabel(Color color, int score) {
        scoreResult = new GLabel("Bricks left: " + bricksLeft
                + "    Your score: " + score);
        scoreResult.setColor(color);
        scoreResult.setFont("Default-15");
        add(scoreResult, getWidth() / 2. -
                scoreResult.getWidth() / 2, getHeight() - scoreResult.getHeight() / 4);
    }
    ...
    private void moveBallAndCollisionsControl() {
        // Create the array of sounds
        ArrayList<double[]> sound = new ArrayList<>();

        // Reading the .wav files into array
        sound.add(0, StdAudio.read("brick.wav"));
        sound.add(1, StdAudio.read("paddle.wav"));
        sound.add(2, StdAudio.read("fail.wav"));
        sound.add(3, StdAudio.read("win.wav"));
        ...
        // outputs the score label into bottom area
        putScoreLabel(Color.BLACK, 0);
        remove(label);
        add(ball);
        ...
        while (true) {
            ...
            if (collider != null) {
                ...
                if (collider == paddle) {
                    StdAudio.play(sound.get(1));
                    vy = -vy;
                } else // collider is brick
                {
                    remove(collider);
                    StdAudio.play(sound.get(0));
                    bricksCounter--;

                    // Increase velocity after each broken brick
                    vx *= INCREASE_FACTOR;
                    vy *= INCREASE_FACTOR;
                    bricksLeft--;

                    // score increase depending on the color of the brick
                    score += brickWeight(collider.getColor());

                    remove(scoreResult);
                    scoreResult = new GLabel("Your score:" + score);
                    putScoreLabel(Color.BLACK, score);

                    // Game end check
                    if (bricksCounter <= 0) {
                        StdAudio.play(sound.get(3));
            ...
            if ((ball.getY() + 2 * BALL_RADIUS) > (getHeight() - PADDLE_Y_OFFSET)) {
                if (crashCounter < NUMBER_TURNS - 1) {
                    StdAudio.play(sound.get(2));
    ...
    /**
     * Determines the "weight" of a brick depending on its color.
     * This value is added to score if the ball hits a brick.
     *
     * @param color The color of the brick.
     * @return The integer value, brick "weight"
     */
    private int brickWeight(Color color) {
    ...
    public void mouseClicked(MouseEvent mouseEvent) {
        //removes visible graphic objects
        remove(ball);
